...
Method 1: Command line (bootstrap on startup)
1. Ensure both peers are on the same network or have public IP addresses and open ports.
2. Start Peer A without --bootstrap: python client.py --port 8333
...
Method 2: GUI top menu bar
1. Start both peers without --bootstrap (or just one as bootstrap is optional).
2. In the top menu bar, click Network → Connect to peer...
3. Enter the IP:port of the other peer (for example 192.168.1.10:8333) and click OK.
4. The nodes will exchange HELLO messages and contacts will appear.

After connecting via either method, click + New Chat, enter the username, and send a message to start chatting.
